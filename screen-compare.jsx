function ScreenCompare({ topic, onReset }) {
  ...
  // Listen for new responses from generateResponse()
  React.useEffect(() => {
...
function DiffChip({ color, label }) {
  return (
    <span style={{
      ...
      padding: '5px 11px', background: color,
...
function FlashcardOverlay({ topic, onClose }) {
  ...
  return (
    <div style={{
      ...
      <div
        style={{ flex: 1, display: 'grid', placeItems: 'center', maxWidth: 900, width: '100%', margin: '0 auto', touchAction: 'pan-y', userSelect: 'none' }}
      ...
      <div className="row" style={{ justifyContent: 'center', alignItems: 'center', gap: 18, maxWidth: 900, margin: '0 auto', width: '100%' }}>
        ...
        <div className="row gap-8">
          {questions.map((_, i) => (
            <span key={i} style={{ width: 14, height: 14, borderRadius: '50%', border: '2.5px solid white', background: i === idx ? 'white' : 'transparent' }}></span>
          ))}
        </div>
